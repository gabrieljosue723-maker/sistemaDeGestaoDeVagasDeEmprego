...
@section('content')
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold mb-6 text-neutral-900">Publicar Nova Vaga</h1>
            ...
            <form method="POST" action="{{ route('vagas.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="descricao" rows="6"
                        ...
                        class="w-full border border-neutral-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-500 @error('descricao') border-neutral-800 @enderror">{{ old('descricao') }}</textarea>
                    @error('descricao')
                        <p class="text-neutral-900 text-xs mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <a href="{{ route('vagas.index') }}" class="text-sm text-neutral-500 hover:text-neutral-800 self-center">
                        Cancelar
                    </a>
                </div>
            </form>
